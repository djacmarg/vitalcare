...
const Navigationbar = () => {
  ...
  const scrollClickEV = () => {
    window.addEventListener("click", (ev) => {
      if (
        ev.target !== dropNavRef.current &&
        ev.target !== buttonNavRef.current
      ) {
        setDropNav(false);
      }
    });
  ...
  scrollClickEV();
  const scrollClickEVT = () => {
    window.addEventListener("click", (evt) => {
      if (
        evt.target !== dropNavRef2.current &&
        evt.target !== buttonNavRef2.current
      ) {
        setDropNav2(false);
      }
    });
  ...
  return (
    <div
      className={
        !sticky
          ? "flex w-full h-22 top-0 sticky  transition-all duration-500 ease-in-out z-10"
          : "flex w-full h-18 top-0 sticky bg-green-600 text-white transition-all duration-500 ease-in-out z-10"
      }
    >
      <div
        className={
      ...
      >
        ...
        <div className="grid p-0 m-0">
          <ul
            className={
              !sticky
                ? "hidden md:flex text-md text-black uppercase "
                : "hidden md:flex text-md text-white uppercase"
            }
          >
